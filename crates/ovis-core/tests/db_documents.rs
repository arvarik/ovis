...
use ovis_core::db::documents::{
    self, ConnectorPlan, DocumentFilter, DocumentUpdate, Position,
};
...
async fn list(
    ...
    let plan = documents::plan_connector_filter(pool, filter).await.unwrap();
...
async fn default_sort_is_newest_first_not_alphabetical_by_url() {
    ...
    let items = list(&pool, &DocumentFilter::default(), SortOrder::UpdatedDesc, 20).await;
...
async fn chunk_count_comes_from_postgres_with_null_distinct_from_zero() {
    ...
    assert_eq!(
        by_id(docs::UNCOUNTED).chunk_count, None,
...
async fn a_document_on_two_connectors_is_counted_once_and_attributed_deterministically() {
    ...
    let filter = DocumentFilter::default();
    let plan = documents::plan_connector_filter(&pool, &filter).await.unwrap();
    let count = documents::count_documents(&pool, &filter, &plan).await.unwrap();
...
async fn source_filtering_is_case_insensitive_and_an_unknown_source_matches_nothing() {
    ...
    assert!(list(&pool, &filter, SortOrder::UpdatedDesc, 20).await.is_empty());
}
...
async fn search_matches_title_and_url_and_escapes_like_metacharacters() {
    ...
    .await;
    assert!(underscore.is_empty(), "'_' matched as a single-char wildcard");
}
...
async fn tags_are_returned_for_a_document_and_bounded() {
    ...
    let tags = documents::get_document_tags(&pool, docs::DELETE_ME, 200)
        .await
        .unwrap();
    let pairs: Vec<(String, String)> = tags.iter().map(|t| (t.key.clone(), t.value.clone())).collect();
...
async fn deleting_a_tagged_document_succeeds_and_clears_every_fk_child() {
    ...
    for (table, column) in [
        ...
        .await
        .unwrap();
        assert_eq!(remaining, 0, "{table} still has rows for the deleted document");
    }
    ...
    let others: i64 = sqlx::query_scalar("SELECT count(*) FROM public.document__tag")
        ...
        .await
        .unwrap();
    assert_eq!(others, 2, "the sweep deleted more than its own document's tags");
}
...
async fn recrawl_risk_is_read_before_the_row_disappears() {
    ...
    assert!(documents::recrawl_risk(&pool, docs::DELETE_ME).await.unwrap());
...
async fn update_merges_metadata_and_never_touches_the_crawl_timestamp() {
    ...
    assert_eq!(metadata["author"], "dave", "the supplied key was not applied");
...
async fn hydration_by_id_preserves_order_independence_and_filters_by_connector() {
    let Some(db) = common::seeded().await else {
        return common::skip("hydration_by_id_preserves_order_independence_and_filters_by_connector");
    };
    let pool = db.pool.clone();
    ...
    let all = documents::documents_by_ids(&pool, &ids, None).await.unwrap();
...
async fn counts_track_filters_and_the_estimate_is_positive() {
    ...
    let total = documents::count_documents(&pool, &DocumentFilter::default(), &ConnectorPlan::Unfiltered)
        .await
        .unwrap();
    assert_eq!(total, 10);
    ...
    sqlx::query("ANALYZE public.document").execute(&pool).await.unwrap();
