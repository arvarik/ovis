...
fn main() -> anyhow::Result<()> {
    ...
    let want_flagged = std::env::args().nth(2).map(|s| s == "flagged").unwrap_or(true);
    ...
    for doc in &docs {
        ...
        if flagged != want_flagged || shown >= 12 { continue; }
        shown += 1;
        let preview: String = doc.text.chars().take(110).collect();
        println!("[{} gates/{} fam] {}\n    gates: {:?}\n    words={} txt={:?}\n",
            f.len(), quality::families_failed(&f), doc.id, f.iter().map(|g| g.code()).collect::<Vec<_>>(),
            m.word_count, preview.replace('\n', "⏎"));
    }
    Ok(())
}
